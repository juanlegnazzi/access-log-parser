...
const { ParserAccessLogService } = require("./parser2");
const fs = require("fs");
...
async function crearArchivoTxt(tiempoTotalDeEspera) {
  ...
  const ipApiService = new IpApiService(httpClient);
  ...
  for (const log of accessLog) {
    ...
    if (ip !== "") {
      ...
      contenidoTxt += `\nINSERT INTO ${tableName} (IP, BROWSER, TIMESTAMP, METHOD, URL, STATUSCODE, COUNTRY, COUNTRYCODE, REGION, REGIONNAME, CITY, ZIP, LAT, LON, TIMEZONE, ISP, ORG, ORG_AS) VALUES ('${log.ip}', '${log.browser}', '${log.timestamp}', '${log.method}', '${log.url}', '${log.statusCode}'`;
