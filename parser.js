...
class ParserAccessLogService {
  parseLogEntry(logEntry) {
    ...
    const logPartsIp = logEntry.split('"');
    // Extrae la información relevante (ajusta esto según el formato de tu registro)
    const ipSplit = logPartsIp.length > 7 ? logPartsIp[7].split(" ") : "";
    const ip = ipSplit.length > 0 ? ipSplit[0].replace(',', "") : "";
    const serverIp = logParts.length > 0 ? logParts[0] : "";
    ...
    return { ip, serverIp, browser, timestamp, method, url, statusCode };
  }
